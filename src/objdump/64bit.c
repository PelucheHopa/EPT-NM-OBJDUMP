...
void display_part64(char *sh_str, FILE* fd, Elf64_Ehdr elf, Elf64_Shdr *sh_table)
{
    for (int i = 1; i < elf.e_shnum; i++) {
        ...
        display_contain64(i, sh_table, fd);
        if (!strcmp(".gnu.build.attributes", (sh_str + sh_table[i].sh_name))) return;
    }
}

void InitSection64(FILE* fd, Elf64_Ehdr elf, Elf64_Shdr *sh_table)
{
    char* buff = NULL;
    ...
    display_part64(buff, fd, elf, sh_table);
}
...
void launch64BIT(char *filename)
{
    ...
    InitSection64(fp, elf_header, sh_table);
    fclose(fp);
}
